...
* Random forest had the highest accuracy of 99.9%, with only 0.01% out of sample error.    

* The gradient boosting machine model had lower accuracy of 97.4%, with 2.6% out of sample error. 

* Linear discriminant analysis achieved only 70.6% accuracy and 29.4% out of sample error. Higher error in this model is expected because it assumes normal distribution.  
...
Out of sample error:
```{r}
1 - confusionMatrix(pred_rf, testing$classe)$overall[1]
```
...
Out of sample error:
```{r}
1 - confusionMatrix(pred_gbm, testing$classe)$overall[1]
```
...
Out of sample error:
```{r}
1 - confusionMatrix(pred_lda, testing$classe)$overall[1]
```
...
Predictions generated by the linear discriminant analysis are 70& identical as predictions generated by the random forest model. Considering only 70% accuracy, or 30% out of sample error of this model, these predictions are highly unreliable. 
```{r}
